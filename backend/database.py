...
def get_admin_alerts():
    alerts = list(
        db.alerts.find({"assigned_to": "admin", "resolved": False}).sort(
            "created_at", -1
        )
    )
    for alert in alerts:
...
def get_guard_profile(guard_id):
    return db.security_guards.find_one({"id": guard_id}, {"_id": 0})


def get_admin_profile(admin_id):
    return db.admins.find_one({"id": admin_id}, {"_id": 0})


def get_resident_profile(resident_id):
    return db.residents.find_one({"id": resident_id}, {"_id": 0})


def get_all_guards():
    return list(db.security_guards.find({}, {"_id": 0}))